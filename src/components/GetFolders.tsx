...
function GetFolders({
  ...
  let folderList = data ?? fetchFiles(folderId, session?.user.email!);
  if (!data && select) folderList = fetchAllFiles(session?.user.email!);

  const sortedList = useMemo(() => {
    const copy = [...folderList];
  ...
  const folders = sortedList.map((folder) => {
