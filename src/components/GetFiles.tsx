...
function GetFiles({
  ...
  let fileList = data ?? fetchFiles(folderId, session?.user.email!);
  if (!data && select) fileList = fetchAllFiles(session?.user.email!);
  ...
  const list = sortedList.map((file) => {
    ...
    return (
      condition && (
        ...
        >
          <div
            className="flex w-full flex-col items-center justify-center overflow-hidden rounded-2xl bg-white shadow-md px-2.5 hover:shadow-xl transition-shadow border border-gray-100"
          >
  ...
  return (
    <>
      {list}
      {showShare && shareFileId && (
        <ShareModal fileId={shareFileId} onClose={() => setShowShare(false)} />
      )}
    </>
  );
}
